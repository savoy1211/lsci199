...
def each_text(texts, text_names):
	for i in range(len(texts)):
		model = AdditiveSmoothingNGramModel(texts[i], n=3, add_tags=True)
		h_words, h_wordset = [], []
		for j in range(1,6):
		...
		pd.DataFrame(df).to_csv("trigram_model_results_windows1to5_"+str(text_names[i]))
		print("Done! Created trigram_model_results_windows1to5_"+str(text_names[i]))
...
each_text([pride_and_prejudice],['pride_and_prejudice'])
