...
def each_text(texts, text_names, n, alpha=0):
	for i in range(len(texts)):
		...
		pd.DataFrame(df).to_csv("NEW_"+str(n)+"gram_alpha"+str(alpha)+"_windows1to5_"+str(text_names[i]))
		print("Done! Created "+"NEW_"+str(n)+"gram_alpha"+str(alpha)+"_windows1to5_"+str(text_names[i]))

# each_text([pride_and_prejudice+moby_dick+hard_times+two_cities],['mpht'], 2, alpha=1)
each_text([pride_and_prejudice],['p'], 2, alpha=1)
