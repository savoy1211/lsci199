...
def each_text(texts, text_names, n, alpha=0):
	for i in range(len(texts)):
		start = time.time()
		model = NGramModel(texts[i], alpha=alpha, n=n, randomize_text=True, sentence_inbound=False)
		h_words, h_wordset = [], []
		for j in range(1,6):
		...
		pd.DataFrame(df).to_csv(str(n)+"gram_random_outbound_alpha"+str(alpha)+"_1to5_"+str(text_names[i]))
		print("Done! Created "+str(n)+"gram_random_outbound_alpha"+str(alpha)+"_1to5_"+str(text_names[i]))
		end = time.time()
		print(end-start)
