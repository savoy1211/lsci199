...
def each_text(training_text, training_text_names, testing_text, testing_text_names, n, alpha=0):
	for i in range(len(training_text)):
		start = time.time()
		model = NGramModel(training_text[0], alpha=alpha, n=n, randomize_text=True, sentence_inbound=False, randomize_sentence_inbound=False, ordered_windows=False)
		test = TestCorpus(testing_text[0], randomize_text=True, sentence_inbound=False, randomize_sentence_inbound=False, ordered_windows=False)
		h_words, h_wordset = [], []
		for j in range(1,6):
		...
		pd.DataFrame(df).to_csv(str(n)+"gram_random_outbound_alpha"+str(alpha)+"_1to5_"+str(training_text_names[0])+str(testing_text_names[0]))
		print("Done! Created "+str(n)+"gram_random_outbound_alpha"+str(alpha)+"_1to5_"+str(training_text_names[0])+str(testing_text_names[0]))
		end = time.time()
		print(end-start)
