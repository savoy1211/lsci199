...
def each_text(texts, text_names, n, alpha=0):
	start = time.time()
	model = NGramModel(texts[0], alpha=alpha, n=n, sentence_inbound=False)
	h_words, h_wordset = [], []
	for j in range(1,6):
	...
	pd.DataFrame(df).to_csv(str(n)+"gram_ordered_text_ordered_window_outbound_alpha"+str(alpha)+"_1to5_"+str(text_names[0]))
	print("Done! Created "+str(n)+"gram_ordered_text_ordered_window_outbound_alpha"+str(alpha)+"_1to5_"+str(text_names[0]))
	end = time.time()
	print(end-start)
	...
	start = time.time()
	model = NGramModel(texts[0], alpha=alpha, n=n, randomize_text=True, sentence_inbound=False, randomize_sentence_inbound=False, ordered_windows=False)
	h_words, h_wordset = [], []
	for j in range(1,6):
		h_words_current, h_wordset_current = survey_text(model, j)
		print("h_words",h_words_current, "h_wordset", h_wordset_current)
	...
	d = { 'h_words': h_words, 'h_wordset': h_wordset}
	df = pd.DataFrame(data=d, dtype=np.float64)
	pd.DataFrame(df).to_csv(str(n)+"gram_random_text_random_window_outbound_alpha"+str(alpha)+"_1to5_"+str(text_names[0]))
	print("Done! Created "+str(n)+"gram_random_text_random_window_outbound_alpha"+str(alpha)+"_1to5_"+str(text_names[0]))
	end = time.time()
	print(end-start)
